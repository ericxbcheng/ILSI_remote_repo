...
# Objective
  ...
  * This is an instruction for the continuous mode in the sampling model v2.0.
  * For more information on the mathematical details, please refer to each module's R markdown file.
...
**Sampling contamination**
  ...
  * `bg_level` = background level of microbial contamination. By default, it is set at 100 CFU.
...
ArgList_default = list(n_contam = n_contam, lims = lims, spread = spread, spread_radius = spread_radius, 
                       ...
                       by = by, LOC = LOC, fun = fun, case = case, m = m, M = M, method_det = method_det, bg_level = bg_level)
```
...
###3. Set a parameter for tuning and a vector of values to tune over.
  ...
  * Parameters available for tuning:  
      + Continuous parameters: `n_contam`, `spread_radius`, `cont_level`, `n_sp`, `n_strata`, `LOC`, `case`, `m`, `M`, `bg_level`
      + Categorical parameters: `method_sp`, `by`, `fun`, `method_det`
...
ArgList_vis[c("case", "M", "method_det")] = NULL

# Produce intermediate outputs
one_iteration = do.call(what = sim_intmed, args = ArgList_vis)[["contam_sp_xy"]] 
```
...
contam_sp_xy = gen_sim_data_new(df_contam = contam_xy, df_sp = sp_xy, dist = dist_contam_sp, 
                                ...
                                bg_level = bg_level, fun = fun )
cover = calc_cover(df_dist = dist_contam_sp, spread_radius = spread_radius, spread = spread)
decision = lot_decision_new(data = contam_sp_xy, case = case, m = m, M = M, spread = spread, 
                            method_det = method_det)
words(x = decision)
...
test1 = sim_intmed(n_contam = n_contam, lims = lims, spread = spread, spread_radius = spread_radius, 
                   ...
                   fun = fun, cont_level = cont_level, bg_level = bg_level)
