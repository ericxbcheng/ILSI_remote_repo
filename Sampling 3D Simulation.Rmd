...
```{r}
# Get the p-value for anova of mean difference in Paccept at 20 ppb among levels of param2
calc_pval_anova = function(data, param3_val){
  mod = aov(formula = Paccept ~ as.factor(param2), data = subset(x = data, subset = (param == 20) & (param3 == param3_val)))
  return(summary(mod)[[1]]$`Pr(>F)`[1])
}

summarize_pval = function(data, n_affected_vec){
  
  pvals = sapply(X = n_affected_vec, FUN = calc_pval_anova, data = data)
  result = data.frame(param3 = n_affected_vec, pval = pvals)
  return(result)
}
```

```{r}
pval_exp1 = summarize_pval(data = exp1_cleaned, n_affected_vec = n_affected_vec)
pval_exp2 = summarize_pval(data = exp2_cleaned, n_affected_vec = n_affected_vec)
```
