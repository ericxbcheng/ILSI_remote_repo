---
title: "Sampling 3D Simulation v2"
...
# Objective
  ...
  To analyze data from 100 iterations of 3D simulation in a 1m x 1m x 1m bin.
...
rds_files = dir(path = paste0(getwd(), "/RDS/3D Simulation"),pattern = "10x10.rds", full.names = TRUE)
...
exp1_cleaned = read.csv(file = "sim_3D_exp1_9_srsxstrsxss_1x10x100_10x10.csv", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
exp2_cleaned = read.csv(file = "sim_3D_exp2_5x10x100_srs_1x10x100_10x10.csv", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
```
...
```{r}
exp_combined = f_combine(data1 = exp1_cleaned, data2 = exp2_cleaned, exp1_param = 9, exp2_param = "srs")
```
...
```{r}
# Experiment all
sens_all = sensitivity::pcc(X = subset(x = exp_combined, select = c("c_hat", "method_sp", "n_affected", "n_sp")), 
...
```{r, echo = FALSE}
sens_all
plot_tornado(data = sens_all, param_labels = c("Aflatoxin concentration", "Sampling strategy", "Clustering", "Number of probes"))
```
