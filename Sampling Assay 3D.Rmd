...
sp_xy = sim_plan_new(method_sp = method_sp, n_sp = 5, spread = spread, lims = lims, radius = spread_radius, container = container)
```
...
3. Visualize aflatoxin distribution in raw sample, submitted sample, work sample, and test sample

```{r, out.width="50%", echo = FALSE, fig.show = "hold", warning = FALSE}
sample_dist(data = contam_sp_xy$samples[[2]], Mc = Mc)
sample_dist(data = sample_dis$raw, Mc = Mc)
sample_dist(data = sample_dis$work, Mc = Mc)
sample_dist(data = sample_dis$test, Mc = Mc)
```

```{r}
assay_draw(data = sample_dis$test, Mc = Mc, method_det = method_det, spread = "discrete")
```
