...
sim_outcome(n_contam = n_contam, xlim = x_lim, ylim = y_lim, n_affected = n_affected, covar_mat = covar_mat, spread_radius = spread_radius, method = "strs", n_sp = n_sp, sp_radius = sp_radius, spread = "continuous", n_strata = n_strata, by = by)
```

2. Run the simulation for `r n_sim` times.

```{r}
ROD = sim_iterate(n_sim = n_sim, n_contam = n_contam, xlim = x_lim, ylim = y_lim, n_affected = n_affected, covar_mat = covar_mat, spread_radius = spread_radius, method = "strs", n_sp = n_sp, sp_radius = sp_radius, spread = "continuous", n_strata = n_strata, by = by)
```
