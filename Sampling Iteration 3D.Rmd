...
3. Iterate the simulation once.

  * The result: `c_true`, `decision`, `mean_raw`, `mean_work`, `mean_test`
...
gen_sim_outcome_new = function(n_contam, lims, spread, spread_radius, method_sp, n_sp, n_strata, 
  ...
  function(...){
    sim_outcome_new(n_contam = n_contam, lims = lims, spread = spread, covar_mat = covar_mat,
                    ...
                    n_sp = n_sp, n_strata = n_strata, by = by, LOC = LOC, fun = fun, case = case, m = m, M = M)
  }
}

f_outcome = gen_sim_outcome_new(n_contam = n_contam, lims = lims, spread = spread, covar_mat = covar_mat,
                         n_affected = n_affected, dis_level = dis_level, method_sp = method_sp, sp_radius = d/2,
                         container = container, L = L, rho = rho, m_kbar = m_kbar, conc_neg = conc_neg, 
                         tox = tox, Mc = Mc, method_det = method_det, diag = TRUE)
...
4. Iterate the simulation n times.

```{r}
result = sim_iterate(n_iter = n_iter, fun = f_outcome)
result2 = clean_new(spread = spread, data = result, diag = TRUE)
```

5. Calculate performance (probability of acceptance, sensitivity and specificity).

```{r}
# Paccept
1 - calc_Prej(decision = result2$decision)

# Sensitivity and specificity
calc_metrics(c_true = result2$c_true, decision = result2$decision, Mc = Mc)
```

# Result

1. Visualization. Show the distribution of true contamination level and the aflatoxin level in raw sample, work portion, and test portion.

```{r, fig.keep = "hold"}
temp = bind_cols(result2) %>%
  dplyr::select(-decision) %>%
  gather(data = ., key = "type", value = "conc")

ggplot(data = temp) +
  geom_histogram(aes(x = conc), bins = 50) +
  labs(x = "Contamination level (ppb)", y = "Number of lots") +
  scale_x_log10() +
  theme_bw() +
