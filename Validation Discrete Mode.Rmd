...
ArgList_default = list(c_hat = c_hat, lims = lims, spread = spread, covar_mat = covar_mat,
                       ...
                       m_sp = m_sp, n_sub = n_sub, unbalanced = unbalanced)
...
1. Show the true contamination level and contamination level from sampling VS input contamination level. This is a quality control step.

```{r}
f_vis(data = result_cleaned, type = "contam")
```
...
```{r}
test1 = sim_iterate_val(n_iter = 5, Args = ArgList_default, seed = 123)
test2 = sim_iterate2_val(n_seed = n_seed, n_iter = n_iter, Args = ArgList_default)
test3 = tune_param_val(Args = ArgList_default, n_seed = n_seed, n_iter = n_iter, param = "c_hat", val = c_hat)
```
