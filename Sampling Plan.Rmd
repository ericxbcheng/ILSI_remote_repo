...
6. Combine the `sp_xy` and `contam_xy`.
    ...
    b) Use the distance and decay function to calculate the contamination contribution from each contamination point.
    
    c) Sum up the contamination contribution from all source points for each sample point.

        $$X_{sp_i} =  X_{cp1}*f(d_{i1})+ X_{cp2}*f(d_{i2})+ ...+X_{cpn}*f(d_{in})$$
        
        , where $X_{sp_i}$ = contamination level of `sample point i`,  
        
        $X_{cpj}$ = contamination level of `contamination point j` (`j` = 1, 2, ..., n),   
    ...
    d) Update the `cont_level` column for sample points by matching row numbers. Add in sampling error, assuming the sampling error follows a         log normal distribution with mean = 0 and sd = 1.
        
        $$X_{sp\_with\_error_i}=X_{sp_i}+\epsilon_i$$
