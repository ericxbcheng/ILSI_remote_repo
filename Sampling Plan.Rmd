...
5. Generate a sampling plan.
    ...
    c) `method = "ss"` generates a systematic sampling plan with `n_sp`, `sp_radius` and `by`.
        ...
        i) Split the field into unit squares.
        
        ii) Calculate `k`. $k = (the number of unit squares) / (the number of sample points)$.
        
        iii) In the first unit square, randomly select a sample point whose coordinates follow a uniform distribution.
        
        iv) Generate the rest of the sample points either along the x-axis or y-axis with the equation: $$x = x_0 + (n-1)k, n = 1, 2, ..., n_sp $$
...
```{r}
contam_sp_xy = gen_sim_data(df_contam = contam_xy, df_sp = sp_xy, spread_radius = spread_radius, LOC = LOC)
```
