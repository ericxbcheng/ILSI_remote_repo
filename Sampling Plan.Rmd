...
```{r}
sp_xy = sim_plan(method = "ss", n_sp = n_sp, xlim = x_lim, ylim = y_lim, radius = sp_radius)
```


6. Combine the `sp_xy` and `contam_xy` by row.
...
```{r, echo = FALSE, eval = FALSE}
# Systematic sampling
plot_sp_dis_ss = sp_draw(method = "ss", data = sp_xy, spread = "discrete", xlim = x_lim, ylim = y_lim)

plot_sp_cont_ss = sp_draw(method = "ss", data = sp_xy, spread = "continuous", xlim = x_lim, ylim = y_lim)
```
