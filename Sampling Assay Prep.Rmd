...
* `L` = length of the probe (m). It can be 5, 6, 8, 10, 12 feet, depending on the container type. Remember to convert it to meters.
...
sp_xy = sim_plan_new(method_sp = "ss", spread = spread, lims = lims, radius = d/2, container = container, depth_ft = depth_ft)
```
...
```{r}
overlay_draw(method_sp = method_sp, data = rbind(contam_xy, sp_xy), spread = spread, xlim = lims$xlim, ylim = lims$ylim)
```
