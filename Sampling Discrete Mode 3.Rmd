---
title: "Sampling Discrete Mode 3"
...
# Objective
  ...
  * This is an instruction for the discrete mode in the sampling model v3.
...
**Sampling strategies**
  ...
  * `L` = length of the probe (m). We assume it's fully inserted to the bottom
      + GIPSA protocol: `L` = 5, 6, 8, 10, 12 feet
...
# 2 tuning parameters
sim_data_2 = tune_param_sec(Args = ArgList_default, var_prim = var_prim, vals_prim = vals_prim, var_sec = var_sec, vals_sec = vals_sec, n_seed = n_seed, n_iter = n_iter)
```

5. Clean up values of probability of acceptance.

```{r, warning = FALSE}
data_cleaned0 = metrics_dis_0(data = sim_data_0)

data_cleaned1 = metrics_dis_n(data = sim_data_1)

data_cleaned2 = metrics_dis_sec(data = sim_data_2, vals_prim = vals_prim, vals_sec = vals_sec, n_seed = n_seed)
```
...
```{r}
plot_tune1(data = data_cleaned1, xlab = "Estimated aflatoxin concentration (ppb)")
```
...
plot_tune2_ribbon(data = data_cleaned2, xlab = "Estimated aflatoxin concentration (ppb)", legend_lab = "Number of samples")
```
