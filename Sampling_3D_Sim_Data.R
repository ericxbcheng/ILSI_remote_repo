...
Args_default_Ex1 = list(c_hat = c_hat, lims = lims, spread = spread, covar_mat = covar_mat,
                       ...
                       sp_radius = sp_radius, n_sp = 7, n_strata = n_strata, by = by, L = L, 
...
for(i in 1:length(Args_list_Ex1)){
  ...
  saveRDS(object = sim_data_Ex1, file = sprintf(fmt = "sim_3x2x1.5_7_0.1_srsxstrsxss_%d_%dx%d.rds", n_affected_vec[i], n_seed, n_iter))  
}
