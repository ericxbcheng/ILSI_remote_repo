...
3. Simulate the contamination center and its spread.
    ...
    c) Generate contamination levels for contamination spots. The contamination levels follow a log normal distribution. The spread of contamination level is defined by a decay function. This function is affected by distance, `spread_radius` and `LOC`.
        
        i) `f_exp()`: $$ f(d) = exp(- \frac {d} {\theta}) $$, where d = distance, $\theta = - \frac {spread radius} {ln(LOC)}$
        
        ii) `f_norm()`: $$ f(d) = exp(- \frac {spread radius^2} {\sigma^2})$$, where d = distance, $\sigma = \sqrt {- {\frac {spreadradius^2} {ln(LOC)}}}$.
...
contam_level_draw(method = "exp", spread_radius = spread_radius, LOC = LOC)

## Choose a normal distribution-like function to describe the contamination spread
contam_level_draw(method = "norm", spread_radius = spread_radius, LOC = LOC)
```
