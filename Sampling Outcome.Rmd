...
4. Evaluate the probability of detection (POD) of the sampling plan.
...
5. Evaluate the assay performance.
...
calc_POD(df_cover = cover_dis, n_sp = n_sp, df_contam = contam_xy, spread = "discrete")
```

* Therefore, we detected `r length(unique(cover_dis$ID_contam))` out of `r nrow(contam_xy)` contamination points. The probability of detection is `r length(unique(cover_dis$ID_contam)) / nrow(contam_xy)`.
...
calc_POD(df_cover = cover_cont, n_sp = n_sp, df_contam = contam_xy, spread = "continuous")
```

* Therefore, `r length(unique(cover_cont$ID_sp))` out of `r n_sp` sample points detected contamination. The probability of detection is `r length(unique(cover_cont$ID_sp)) / n_sp`.
